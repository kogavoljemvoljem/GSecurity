...
$Backup     = "C:\ProgramData\Antivirus\Backup"
$LogFile    = "C:\ProgramData\Antivirus\log.txt"
$LastFile   = "C:\Quarantine\.last"
...
Log "Antivirus started - monitoring .exe, .dll, .sys and .winmd"
...
function Move-ToQuarantine($file) {
    ...
    Log "QUARANTINED -> $q"
    [System.Windows.Forms.MessageBox]::Show("$name`nQuarantined!", "Antivirus", "OK", "Warning") | Out-Null
}
...
function Undo-LastQuarantine {
    ...
    [System.Windows.Forms.MessageBox]::Show("Last file restored!", "Antivirus", "OK", "Information") | Out-Null
}
...
Log "Antivirus started in fully automatic silent mode (no pop-ups)"
